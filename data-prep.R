...
images <-
  tibble(path = list.files("data-raw", recursive = TRUE)) %>%
...
# create train-validaation-test data meta ---------------------------------

data_train <-
  tibble(path = list.files("data/train", recursive = TRUE)) %>%
  separate(path, into = c("class", "name"), sep = "/") %>%
  mutate(path = glue("data/train/{class}/{name}"))

data_val <-
  tibble(path = list.files("data/validation", recursive = TRUE)) %>%
  separate(path, into = c("class", "name"), sep = "/") %>%
  mutate(path = glue("data/validation/{class}/{name}"))

data_test <-
  tibble(path = list.files("data/test", recursive = TRUE)) %>%
  separate(path, into = c("class", "name"), sep = "/") %>%
